...
/*  Struct and routines for working with complex rational functions.          */
...
#define NF_REAL_RAT_NEWTON_FRACTAL(num, den, name)                             \
    ...
    const unsigned int dendeg = sizeof(den)/sizeof(den[0]) - 1U;               \
                                                                               \
    /*  Create polynomials from the given coefficients.                      */\
    const struct nf_real_poly num_poly = {num, numdeg};                        \
    const struct nf_real_poly den_poly = {den, dendeg};                        \
                                                                               \
    /*  Create a rational function from the two polynomials.                 */\
    const struct nf_real_rational rat = {num_poly, den_poly};                  \
                                                                               \
    /*  Indices for the x and y axes of the PPM file.                        */\
    unsigned int x, y;                                                         \
                                                                               \
    ...
    {                                                                          \
        ...
        return -1;                                                             \
                                                                               ...
                                                                               \
    /*  Find the roots for the polynomial.                                   */\
    roots = nf_real_poly_roots(&num_poly);                                     \
                                                                               \
    /*  Check if the root finding function failed.                           */\
    if (!roots)                                                                \
    {                                                                          \
        /*  Close the ppm file, print an error, and return.                  */\
        puts("nf_real_poly_roots returned NULL. Aborting.");                   \
        nf_ppm_close(&ppm);                                                    \
        return -1;                                                             \
    }                                                                          \
                                                                               \
    ...
    {                                                                          \
        /*  Close the ppm, print an error, and destroy the root struct.      */\
        puts("nf_real_poly_roots failed to find roots. Aborting.");            \
        nf_complex_roots_destroy(&roots);                                      \
        nf_ppm_close(&ppm);                                                    \
        return -1;                                                             \
    }                                                                          \
    ...
    {                                                                          \
        ...
        {                                                                      \
            ...
            z.imag = imag;                                                     \
                                                                               \
            /*  Perform Newton's method for rat with guess z0 = z.           */\
            iters = nf_newton_real_rational(&rat, &z);                         \
                                                                               \
            /*  If Newton's method fails the default color is black.         */\
            c = nf_black;                                                      \
                                                                               \
...
#define NF_REAL_SPHERICAL_NEWTON_FRACTAL(coeffs, name)                         \
    ...
    const struct nf_vec3 u = nf_vec3_normalize(&nf_camera_pos);                \
                                                                               \
    /*  The degree can be computed by the length of the array minus 1.       */\
    const unsigned int degree = sizeof(coeffs)/sizeof(coeffs[0]) - 1U;         \
                                                                               \
